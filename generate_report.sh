...
usage() {
    ...
    echo "Pre-requisite: Built ezmq-protocol-cpp library in debug mode"
    echo "Usage: generate_report.sh <options>"
    echo "Options:"
    echo "      -h / --help                                  :  Display help and exit"
    echo "      -c                                           :  Clean C EZMQ Repository; Should be used to clean existing repository"
...
generate_report_EZMQ()
{
    ...
    LD_LIBRARY_PATH="${EZMQ}/dependencies/protocol-ezmq-cpp/out/${EZMQ_TARGET_OS}/${EZMQ_TARGET_ARCH}/debug"
    ...
    gcovr -r . \
        ...
        -e "unittests.*" \
		-e "samples.*" \
		-e "dependencies.protocol-ezmq-c.*" \
